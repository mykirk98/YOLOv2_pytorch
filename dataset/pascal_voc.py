...
class pascal_voc(imdb):
    ...
    def evaluate_detections_with_train(self, all_boxes, output_dir=None):
        self._write_voc_results_file_with_train(all_boxes)
        map = self._do_python_eval_with_train(output_dir)
        if self.config['cleanup']:
            for cls in self._classes:
                if cls == '__background__':
                    continue
