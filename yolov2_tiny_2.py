...
class Yolov2(nn.Module):
    ...
    # def manual_weight_update(model, learning_rate=0.0001, momentum = 0.9, weight_decay=0.0005):
    #     with torch.no_grad():     
    #         for param in model.parameters():
    #             if param.grad is not None:
    #                 if not hasattr(param, 'momentum'):
    #                     param.momentum = torch.zeros_like(param.data)
    #                 param.momentum = momentum * param.momentum + param.grad.data
    #                 param.data = param.data - learning_rate * param.momentum
    #                 param.data = param.data - learning_rate * weight_decay * param.data
                    
                    # param.momentum = (momentum * param.data)  - (1 - momentum) * param.grad
                    # param.data = param.data - learning_rate * (param.momentum + weight_decay)
                    # param.data = param.data - learning_rate * param.grad
                    # param.data -= learning_rate * (param.grad + weight_decay * param.data)  
                    # param.momentum = momentum * param.momentum + param.grad
       
    # def manual_weight_update(model, learning_rate=0.0001, momentum = 0.9, weight_decay=0.0005):
    #     with torch.no_grad():     
    #         for param in model.parameters():
    #             if param.grad is not None:  
    #                 # weight_decay = (learning_rate * weight_decay / 8) * param.data
    #                 if not hasattr(param, 'momentum'):
    #                     param.momentum = torch.zeros_like(param.data)
    #                 param.momentum = momentum * param.momentum + param.grad.data
    #             param.data = param.data - weight_decay - learning_rate * param.momentum 
                
                    
    def manual_weight_update(model, learning_rate=0.0001, momentum = 0.9, weight_decay=0.0005, dampening = 0):
        with torch.no_grad():       
            for param in model.parameters():
                ...
                # old = torch.clone(param.data).detach()
                
                if weight_decay != 0:
                    d_p = d_p + weight_decay * param.data  # torch.add()
                    # apply learning ate 
                ...
                # if weight_decay != 0:
                #     d_p = d_p + weight_decay * old  # torch.add()
                 
                    # param_state = model.state_dict[param.data]
                    # if 'momentum buffer' not in param_state:
                    #     buffer = param_state['momentum buffer'] = torch.zeros_like(param.data)
                    #     buffer = buffer * (momentum + d_p)
                    # else:
