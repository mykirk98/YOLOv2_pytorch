...
class Yolov2(nn.Module):
    ...
    # def manual_weight_update(self, learning_rate=0.001, num_epochs= 160, iter_per_epoch=2068):
    #     # epoch_counter = 0
    ...
    #             grads_before_update.append(param.grad.view(-1)[:10].clone()) 
    ...
    #             for param in self.parameters():
    #                 if param.grad is not None:
    #                     grads_after_update.append(param.grad.view(-1)[:10].clone())
                        
                        
    #             for i, (grad_b, grad_a) in enumerate(zip(grads_before_update, grads_after_update)):

    #                 print(f"Gradients before update: {grad_b}")
    #                 print(f"Gradients after update: {grad_a}")           
