...
image_file_path = "3class_bdd_crop/original_images/train/0af07355-56e42b91.jpg"
...
# Raed label
boxes, GT_classes = ReadLabels(label_file_path=label_file_path)
boxes, GT_classes = np.array(boxes), np.array(GT_classes)  # convert to numpy array
...
for box in boxes:
    ...
    # most_left, most_top, most_right, most_bottom = most_left, most_top, most_right, most_bottom
...
GT_classes = GT_classes[keep]         # if {width == 0 | height == 0} exist, 'classes' will change
...
image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)      # If you want to implement in the yolov2-pytorch, you must change the variable name different with parameter "image"
...
# cv2.imwrite(filename="./result/7. Final result.jpg", img=image)